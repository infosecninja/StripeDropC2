...
- **🛡️ Self-Protection**: Automatic relocation to legitimate-looking Windows paths
...
4. **⚠️ CRITICAL: Regenerate implant encryption key:**
```bash
python3 regenerate_key.py
```

**🔴 YOU MUST RUN THIS BEFORE EVERY BUILD!** This randomizes the XOR key and encrypted strings in `Implant.cs`, changing the binary signature to evade detection. Skipping this step means:
...
|---------|---------------|-----------|----------------|---------------|
| **LOW** | 2-5s | 15-30s | ⚠️ Very High | Instant |
| **BALANCED** | 5-15s | 25-35s | ⚠️ Moderate | Fast |
...
⚠️ **API Key Exposure**: If key is compromised, entire C2 is burned  
⚠️ **Stripe Rate Limits**: Test mode limited to ~100 requests/second  
⚠️ **Billing Traces**: Live mode creates financial audit trail  
⚠️ **Network Anomalies**: High-frequency Stripe API calls may be unusual for target org  
⚠️ **Metadata Limits**: 500 chars per field, requires chunking for large data  
